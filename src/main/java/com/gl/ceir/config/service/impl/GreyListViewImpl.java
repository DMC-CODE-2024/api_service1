...
import com.gl.ceir.config.repository.app.GreyListRepo;
...
private static final Logger logger = LogManager.getLogger(CheckIMEIParamImpl.class);
	...
	public Page<GreyListView> filterBlackListIMEI(FilterRequest filterRequest, Integer pageNo,
			Integer pageSize) {
		try {
...
//			createdOn,taxPaidStatus,quantity,deviceQuantity,supplierName,consignmentStatus
			...
			Page<GreyListView> page = greyListRepo.findAll(buildSpecification_system(filterRequest).build(), pageable);
			...
			auditTrailRepository.save(new AuditTrail(filterRequest.getUserId(), filterRequest.getUserName(),
					...
					Long.valueOf(filterRequest.getFeatureId()), Features.Grey_LIST_IMEI, SubFeatures.VIEW_ALL, "", "NA",
	...
	private GenericSpecificationBuilder<GreyListView> buildSpecification_system(FilterRequest filterRequest) {

		GenericSpecificationBuilder<GreyListView> sb = new GenericSpecificationBuilder<GreyListView>(
	...
	public FileDetails exportFile_System(FilterRequest filterRequest) {
		...
		try {
			List<GreyListView> checkIMEIResponseParamExport = getAll_greyIMEI(filterRequest);
			fileName = LocalDateTime.now().format(dtf2).replace(" ", "_") + "_Tracked List.csv";
			...
			if (!checkIMEIResponseParamExport.isEmpty()) {
				...
				for (GreyListView systemConfigurationDB : checkIMEIResponseParamExport) {
			...
			auditTrailRepository.save(new AuditTrail(filterRequest.getUserId(), filterRequest.getUserName(),
					Long.valueOf(filterRequest.getUserTypeId()), filterRequest.getUserType(),
					Long.valueOf(filterRequest.getFeatureId()), Features.Grey_LIST_IMEI, SubFeatures.EXPORT, "", "NA",
					filterRequest.getRoleType(),filterRequest.getPublicIp(),filterRequest.getBrowser()));
	...
	private List<GreyListView> getAll_greyIMEI(FilterRequest filterRequest) {
		try {
			List<GreyListView> list = greyListRepo.findAll(
	...
	public GreyListView findBySno(GreyListView greyListModel) {
		try {
			
			GreyListView greyListParam = greyListRepo.getBySno(greyListModel.getSno());
			auditTrailRepository.save(new AuditTrail(greyListModel.getUserId(), greyListModel.getUserName(),
					Long.valueOf(greyListModel.getUserTypeId()), "system",
					Long.valueOf(greyListModel.getFeatureId()), Features.Grey_LIST_IMEI, SubFeatures.VIEW, "", "NA",
					greyListModel.getRoleType(),greyListModel.getPublicIp(),greyListModel.getBrowser()));
			logger.info("grey list view by id : successfully inserted in Audit trail.");
		return greyListParam;
		} catch (Exception e) {
			logger.info("Exception found=" + e.getMessage());
			throw new ResourceServicesException(this.getClass().getName(), e.getMessage());
		}
