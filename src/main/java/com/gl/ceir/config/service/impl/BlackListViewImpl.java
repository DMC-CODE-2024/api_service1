...
import com.gl.ceir.config.repository.app.BlackListRepo;
...
@Service
public class BlackListViewImpl {
	
	private static final Logger logger = LogManager.getLogger(BlackListViewImpl.class);
	...
	public Page<BlackListModel> filterBlackListIMEI(FilterRequest filterRequest, Integer pageNo,
			Integer pageSize) {
		try {
...
//			createdOn,taxPaidStatus,quantity,deviceQuantity,supplierName,consignmentStatus
			...
			Page<BlackListModel> page = blackListRepo.findAll(buildSpecification_system(filterRequest).build(), pageable);
			...
			auditTrailRepository.save(new AuditTrail(filterRequest.getUserId(), filterRequest.getUserName(),
					...
					Long.valueOf(filterRequest.getFeatureId()), Features.Black_LIST_IMEI, SubFeatures.VIEW_ALL, "", "NA",
					filterRequest.getRoleType(),filterRequest.getPublicIp(),filterRequest.getBrowser()));
			logger.info("Black_LIST_IMEI : successfully inserted in Audit trail.");
			return page;
	...
	private GenericSpecificationBuilder<BlackListModel> buildSpecification_system(FilterRequest filterRequest) {

		GenericSpecificationBuilder<BlackListModel> sb = new GenericSpecificationBuilder<BlackListModel>(
	...
	public FileDetails exportFile_System(FilterRequest filterRequest) {
		...
		try {
			List<BlackListModel> checkIMEIResponseParamExport = getAll_blackIMEI(filterRequest);
			fileName = LocalDateTime.now().format(dtf2).replace(" ", "_") + "_Blocked List.csv";
			...
			if (!checkIMEIResponseParamExport.isEmpty()) {
				...
				for (BlackListModel systemConfigurationDB : checkIMEIResponseParamExport) {
			...
			auditTrailRepository.save(new AuditTrail(filterRequest.getUserId(), filterRequest.getUserName(),
					Long.valueOf(filterRequest.getUserTypeId()), filterRequest.getUserType(),
					Long.valueOf(filterRequest.getFeatureId()), Features.Black_LIST_IMEI, SubFeatures.EXPORT, "", "NA",
					filterRequest.getRoleType(),filterRequest.getPublicIp(),filterRequest.getBrowser()));
	...
	private List<BlackListModel> getAll_blackIMEI(FilterRequest filterRequest) {
		try {
			List<BlackListModel> list = blackListRepo.findAll(
	...
	public BlackListModel findBySno(BlackListModel blackListModel) {
		try {
			
			BlackListModel blackListParam = blackListRepo.getBySno(blackListModel.getSno());
			auditTrailRepository.save(new AuditTrail(blackListModel.getUserId(), blackListModel.getUserName(),
					Long.valueOf(blackListModel.getUserTypeId()), "system",
					Long.valueOf(blackListModel.getFeatureId()), Features.Black_LIST_IMEI, SubFeatures.VIEW, "", "NA",
					blackListModel.getRoleType(),blackListModel.getPublicIp(),blackListModel.getBrowser()));
			logger.info("black list view by id : successfully inserted in Audit trail.");
