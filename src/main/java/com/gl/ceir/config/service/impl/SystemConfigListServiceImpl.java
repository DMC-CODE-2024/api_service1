...
public class SystemConfigListServiceImpl {
	...
	public GenricResponse saveSystemConfigList(SystemConfigListDb systemConfigListDb) {
		try {
			...
			auditTrailRepository.save(new AuditTrail(systemConfigListDb.getUserId(), systemConfigListDb.getUsername(), 0L,systemConfigListDb.getUserType(), 0L, 
					Features.FIELD_MANGEMENT, SubFeatures.Save, "","NA",systemConfigListDb.getUserType(),systemConfigListDb.getPublicIp(),systemConfigListDb.getBrowser()));
	...
	public GenricResponse updateSystemConfigList(SystemConfigListDb systemConfigListDb) {
		try {
			...
			auditTrailRepository.save(new AuditTrail(systemConfigListDb.getUserId(), systemConfigListDb.getUserName(), 0L, systemConfigListDb.getUserType(), 0L, 
					Features.FIELD_MANGEMENT, SubFeatures.UPDATE, "","NA",systemConfigListDb.getUserType(),systemConfigListDb.getPublicIp(),systemConfigListDb.getBrowser()));
			return new GenricResponse(0);
...
//				Features.CONFIG_LIST, SubFeatures.VIEW, ""));
			...
			auditTrailRepository.save(new AuditTrail(filterRequest.getUserId(), filterRequest.getUserName(), 0L,filterRequest.getUserType(), 0L, 
					Features.FIELD_MANGEMENT, SubFeatures.FILTER, "","NA",filterRequest.getUserType(),filterRequest.getPublicIp(),filterRequest.getBrowser()));
	...
	public GenricResponse findById(FilterRequest filterRequest) {
		try {
			...
			auditTrailRepository.save(new AuditTrail(filterRequest.getUserId(), filterRequest.getUserName(), 0L,filterRequest.getUserType(), 0L, 
					Features.FIELD_MANGEMENT, SubFeatures.VIEW, "","NA",filterRequest.getUserType(),filterRequest.getPublicIp(),filterRequest.getBrowser()));
...
//			
	...
	public GenricResponse deleteValue(FilterRequest filterRequest) {
		try {
			...
			auditTrailRepository.save(new AuditTrail(filterRequest.getUserId(), filterRequest.getUserName(), 0L,filterRequest.getUserType(), 0L, 
					Features.FIELD_MANGEMENT, SubFeatures.DELETE, "","NA",filterRequest.getUserType(),filterRequest.getPublicIp(),filterRequest.getBrowser()));
