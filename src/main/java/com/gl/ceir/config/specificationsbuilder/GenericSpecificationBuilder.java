...
public class GenericSpecificationBuilder<T> {
	...
	public <T> Specification joinWithUser(SearchCriteria searchCriteria) {
		if (Objects.nonNull(searchCriteria.getValue())) {
			return (root, query, cb) -> {
				Join<T, User> join_User = root.join("user".intern());
				return cb.like(join_User.get(searchCriteria.getKey()), "%" + searchCriteria.getValue().toString() + "%");
			};
		}
		return null;
	}
	...
	public <T> Specification joinWithMultiple(SearchCriteria searchCriteria) {
		if (Objects.nonNull(searchCriteria.getValue())) {
			return (root, query, cb) -> {
				Join<T, PoliceStationDb> join = root.join("policeStationDb".intern());
				return cb.equal(join.get(searchCriteria.getKey()), Long.valueOf(searchCriteria.getValue().toString()));
			};
		}
		return null;
	}

	public Specification<UserProfile> hasGroupId(Long groupId) {
		return (root, query, criteriaBuilder) -> {
			Join<UserProfile, UserGroupMembership> membershipJoin = root.join("userGroupMemberships", JoinType.INNER);
			return criteriaBuilder.equal(membershipJoin.get("groupId"), groupId);
		};
	}
}